...
describe("buildWidgetLines", () => {
	...
	describe("without theme", () => {
		it("renders a single new file", () => {
			const m = trackedMap(
				trackedFile({ path: "a.ts", displayPath: "a.ts", kind: "new", added: 3, removed: 0, updatedAt: 100 }),
			);
		...
		it("renders a single edited file with removals", () => {
			const m = trackedMap(
				trackedFile({ path: "a.ts", displayPath: "a.ts", kind: "edited", added: 1, removed: 2, updatedAt: 200 }),
			);
			const lines = buildWidgetLines(m)!;
			assert.equal(lines.length, 1);
		...
		it("sorts by updatedAt descending", () => {
			const m = trackedMap(
				trackedFile({ path: "old.ts", displayPath: "old.ts", kind: "edited", added: 1, removed: 0, updatedAt: 50 }),
				trackedFile({ path: "mid.ts", displayPath: "mid.ts", kind: "edited", added: 2, removed: 0, updatedAt: 100 }),
				trackedFile({ path: "new.ts", displayPath: "new.ts", kind: "new", added: 5, removed: 0, updatedAt: 200 }),
			);
			const lines = buildWidgetLines(m)!;
		...
		it("omits overflow when below max", () => {
			const m = trackedMap(
				trackedFile({ path: "a.ts", displayPath: "a.ts", updatedAt: 100 }),
			);
			const lines = buildWidgetLines(m)!;
			assert.equal(lines.length, 1);
			assert(!lines.some((l) => l.startsWith("…")));
		...
		it("renders edited file with only removals", () => {
			const m = trackedMap(
				trackedFile({ path: "del.ts", displayPath: "del.ts", kind: "edited", added: 0, removed: 10, updatedAt: 100 }),
			);
			const lines = buildWidgetLines(m)!;
	...
	describe("with theme", () => {
		...
		it("renders new file with muted tag and colored counts", () => {
			const m = trackedMap(
				trackedFile({ path: "a.ts", displayPath: "a.ts", kind: "new", added: 5, removed: 0, updatedAt: 100 }),
			);
		...
		it("renders edited file with Δ tag", () => {
			const m = trackedMap(
				trackedFile({ path: "b.ts", displayPath: "b.ts", kind: "edited", added: 0, removed: 3, updatedAt: 200 }),
			);
			const lines = buildWidgetLines(m, t)!;
		...
		it("renders mixed added/removed with both colors", () => {
			const m = trackedMap(
				trackedFile({ path: "mix.ts", displayPath: "mix.ts", kind: "edited", added: 3, removed: 2, updatedAt: 100 }),
			);
			const lines = buildWidgetLines(m, t)!;
		...
		it("renders zero added/removed with text color only", () => {
			const m = trackedMap(
				trackedFile({ path: "z.ts", displayPath: "z.ts", kind: "edited", added: 0, removed: 0, updatedAt: 100 }),
			);
			const lines = buildWidgetLines(m, t)!;
...
describe("buildTrackedFileLabel", () => {
	...
	it("renders long paths correctly", () => {
		assert.equal(
			buildTrackedFileLabel(
				trackedFile({ kind: "edited", displayPath: "very/deeply/nested/directory/structure/file.ts" }),
			),
...
describe("buildTrackedFileDescription", () => {
	it("formats added and removed", () => {
		assert.equal(
			buildTrackedFileDescription(trackedFile({ added: 5, removed: 3 })),
			"+5/-3",
		);
	});
	...
	it("formats zero changes", () => {
		assert.equal(
			buildTrackedFileDescription(trackedFile({ added: 0, removed: 0 })),
			"+0/-0",
		);
	});
	...
	it("formats only additions", () => {
		assert.equal(
			buildTrackedFileDescription(trackedFile({ added: 10, removed: 0 })),
			"+10/-0",
		);
	});
	...
	it("formats only removals", () => {
		assert.equal(
			buildTrackedFileDescription(trackedFile({ added: 0, removed: 7 })),
			"+0/-7",
		);
	});
});
...
describe("buildSelectItems", () => {
	...
	it("appends tracked files after the separator in updatedAt descending order", () => {
		const m = trackedMap(
			trackedFile({ path: "old.ts", displayPath: "old.ts", kind: "edited", added: 1, removed: 2, updatedAt: 50 }),
			trackedFile({ path: "new.ts", displayPath: "new.ts", kind: "new", added: 10, removed: 0, updatedAt: 200 }),
		);
	...
	it("uses file path as the select value", () => {
		const m = trackedMap(
			trackedFile({ path: "very/deep/file.ts" }),
		);
		const items = buildSelectItems(m);
